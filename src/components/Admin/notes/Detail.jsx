...
function Detail() {
    ...
    useEffect(() => {
        if (param.id) {
            const fetchNote = async () => {
                const res = await getNoteById(param.id);
                if (res.success) {
                    updateData(res);
                }
            };
    ...
    const onSubmit = async (data) => {
        const res = await updateNote(param.id, formatData(data));
        if (res.success) {
    ...
    function updateData(res) {
        fieldsToReset.forEach(fieldName => {
            if (fieldName === "thoiGianPhatHien" || fieldName === "ngayKhamBenh") {
                setValue(fieldName, new Date(res.data[fieldName]));
            } else if(fieldName === "tienSuBenh") {
                setInputTSBenh(res.data[fieldName]);
                setValue(fieldName, res.data[fieldName].map(e => ({ label: e, value: e })));
            } else if(res.data[fieldName]) {
                if (Array.isArray(res.data[fieldName])) {
                    setValue(fieldName, res.data[fieldName].map(e => ({ label: e, value: e })));
                } else
                    setValue(fieldName, res.data[fieldName]);
            }
            setInputDate(new Date(res.data.thoiGianPhatHien));
        });
    }
    ...
    return (
        <div className="mx-3 sm:mx-10 overflow-hidden">
            <ToastContainer />
            <div className="text-center font-bold text-2xl my-4">Chỉnh sửa thông tin</div>
